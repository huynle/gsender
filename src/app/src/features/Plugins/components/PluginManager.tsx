        ...
        if (isElectron()) {
            (window as any).ipcRenderer.on(
                ...
                async (_: any, directory: string, indexFile: string) => {
                    const res =
                        await api.plugins.readImportedManifest(directory);
                    ...
                    Confirm({
                        ...
                        content: (
                            <div className="flex flex-col h-full">
                                {hasDynamicImport && (
                                    <>
                                        <p className="font-bold">
                                            The actions this plugin runs cannot
                                            be verified.
                                        </p>
                                        <p className="font-bold">
                                            Not all permissions needed may be
                                            listed here.
                                        </p>
                                ...
                                {permissions.length > 0 ? (
                                    <>
                                        <p>
                                            The plugin {plugin.name} needs the
                                    ...
                                    <p>
                                        The plugin {plugin.name} does not need
                                        any permissions.
                                    </p>
                                )}
                                {declaredParsers.length > 0 && (
                                    <>
                                        <hr />
                                        <p>
                                            It watches your machine's responses
                                            for these patterns:
                                        </p>
                                        <ul>
                                            {declaredParsers.map(
                                ...
                                {parserErrors.length > 0 && (
                                    <>
                                        <p className="font-bold text-amber-500">
                                            Some of this plugin's parsers were
                                            rejected and will not run:
                                        </p>
                                ...
                                <p>
                                    Press Authorize to continue importing this
                                    plugin.
                                </p>
                            </div>
                        ),
                        ...
                        onConfirm: async () => {
                            ...
                            api.plugins
                                .writePermissions(directory, wire)
                                .then((res) => {
                                    if (res.status !== 200) {
        ...
        >
            <div className="flex flex-col gap-4">
                ...
                <p className="text-sm text-gray-500 dark:text-content-muted">
                    ...
                    installing or enabling a plugin, restart gSender for mount
                    routes to apply.
                </p>
                ...
                {restartRequired && (
                    <div className="rounded-md border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-700 dark:bg-amber-950 dark:text-amber-100">
                        Restart gSender to apply plugin changes. Mount routes
                        are registered when the server starts.
                    </div>
                )}
