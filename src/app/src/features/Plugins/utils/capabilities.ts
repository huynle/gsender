...
export const buildGrantFromScan = (
    ...
    const permissions = [
        ...new Set(scanned.flatMap((name) => permissionsMap.get(name) ?? [])),
    ];
    const requestTypes = [
        ...new Set(scanned.flatMap((name) => requestTypesMap.get(name) ?? [])),
    ];
    const topics = [
        ...new Set(
...
export const mergeManifestParserGrant = (
    ...
    return {
        permissions: [
            ...new Set([...grant.permissions, 'machine:parse' as const]),
        ],
        wire: {
            ...grant.wire,
