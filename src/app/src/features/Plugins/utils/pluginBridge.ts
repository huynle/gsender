...
const handleBridgeRequest = async (
    ...
    if (STORAGE_REQUEST_TYPES.has(request.type) && !pluginId) {
        throw new Error(
            'Unable to resolve plugin identity for storage request',
        );
    }
    ...
    switch (request.type) {
        ...
        case 'viewer:screen-to-world': {
            ...
            const { px, py } = (request.payload ?? {}) as {
                px: number;
                py: number;
            };
        ...
        case 'viewer:pick:arm': {
            ...
            if (!machineIsConnectedAndIdle()) {
                throw new Error(
                    'Machine must be connected and idle to pick a point',
                );
            }
...
const ensureHostListeners = () => {
    ...
    controller.addListener(
        ...
        (match: PluginParserMatch) => {
            ...
            const forPlugin = {
                ...(parserSnapshots.get(match.pluginId) ?? {}),
            };
