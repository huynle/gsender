...
function ATCContent({ mode }: { mode: DrawerMode }) {
    ...
    const linesReceived = useTypedSelector((s: RootState) =>
        get(s, 'controller.sender.status.received', 0),
    );
    const workflowState = useTypedSelector(
        (s: RootState) => s.controller.workflow.state,
    );
    const toolTableData = useTypedSelector(
        (s: RootState) => s.controller.settings.toolTable,
    );
    ...
    const effectiveRackSize =
        reportedRackSize > 0
            ? reportedRackSize
            : Object.values(toolTableData || {}).length;
    const toolTable = mapToolNicknamesAndStatus(
        toolTableData,
        effectiveRackSize,
    );
    ...
    useEffect(() => {
        ...
        if (active && linesReceived > active.endLine) {
            setActiveToolIndex((i) =>
                Math.min(i + 1, timelineTools.length - 1),
            );
        }
    }, [linesReceived]);
    ...
    useEffect(() => {
        ...
        itemRefs.current[activeToolIndex]?.scrollIntoView({
            behavior: 'smooth',
            block: 'nearest',
        });
    ...
    const handleConfirmRemap = (fromTool: number, toTool: number) => {
        setMappings((prev) => {
            ...
            if (fromTool === toTool) {
                next.delete(fromTool);
            } else {
                next.set(fromTool, toTool);
            }
    ...
    return (
        <div
            className={`flex flex-col gap-3 px-3 py-2 h-full min-h-0 ${mode === 'minimal' ? 'justify-center' : ''}`}
        >
            <ATCStartValidations
                show={showValidator}
                        ...
                        <div className="flex-1 overflow-y-auto no-scrollbar min-h-0">
                            {timelineTools.map((tool, index) => {
                                ...
                                const isManual = allowManualBadge
                                    ? (toolInfo?.isManual ??
                                      (effectiveRackSize > 0
                                          ? lookupId > effectiveRackSize
                                          : false))
                                    : false;
                                return (
                                    <div
                                        key={`${tool.index}-${tool.toolNumber}-${tool.startLine ?? index}`}
                                        ref={(el) => {
