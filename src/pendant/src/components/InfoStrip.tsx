...
function Divider() {
    return <span className="w-px self-stretch bg-gray-200 dark:bg-outline" />;
}
...
export default function InfoStrip() {
    ...
    const currentTool = useTypedSelector(
        (state: RootState) =>
            (state.controller.state.status as any)?.currentTool,
    );
        ...
        <div className="relative z-40 flex flex-wrap items-center gap-4 px-3 md:px-4 py-1.5 bg-white border-b border-gray-200 dark:bg-surface-base dark:border-outline shrink-0 text-xs sm:text-sm text-gray-500 dark:text-content-muted">
            ...
            <span className="flex flex-wrap items-center gap-3.5">
                ...
                <span className="flex flex-col items-center leading-tight">
                    ...
                    <span
                        className={cx(
                            'font-mono text-[11px] leading-[1.4] text-gray-700 dark:text-content-primary',
                            {
                                'text-red-500 dark:text-red-500 animate-pulse':
                                    spindleActive && !isLaserMode,
                                'text-purple-500 dark:text-purple-500 animate-pulse':
                                    spindleActive && isLaserMode,
                            },
                        )}
                    >
