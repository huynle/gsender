...
export default function MacrosPanel({ mode }: Props) {
    ...
    const handleEdit = async (data: {
        ...
        if (!data.id) return;
        try {
            await api.macros.update(data.id, {
                name: data.name,
    ...
    const handleExport = async () => {
        try {
            ...
            const data = records.map(({ id, name, content, description }) => ({
                id,
                name,
                content,
                description,
    ...
    const macroToDelete = confirmDeleteId
        ? macros.find((m) => m.id === confirmDeleteId)
        : null;
            ...
            >
                <DialogContent>
                    ...
                    <p className="text-sm text-gray-600 dark:text-content-muted">
                        Delete <strong>{macroToDelete?.name}</strong>? This
                        cannot be undone.
                    </p>
                    <DialogFooter>
                        <button
