...
export default function ProbePanel({ mode }: Props) {
    ...
    const calcToolDiameter = (
        newU?: UNITS_EN,
        newPT?: PROBE_TYPES_T,
    ): number => {
        const newProbeType = newPT || probeType;
        const newUnits = newU || units;
        if (newProbeType === PROBE_TYPE_AUTO || newProbeType === 'Tip')
            return 0.0;
    ...
    const actions: Actions = {
        ...
        setProbeConnectivity: (cm: boolean): void => {
            setConnectionMade(cm);
        },
        ...
        generatePossibleProbeCommands: (): ProbeCommand[] => {
            ...
            const functions = {
                z: true,
                y: selectedProfile.touchplateType !== TOUCHPLATE_TYPE_ZERO,
                x: selectedProfile.touchplateType !== TOUCHPLATE_TYPE_ZERO,
            };
    ...
    const generateProbeCommands = (): string[] => {
        ...
        if (units === METRIC_UNITS) {
            ...
            zThickness = {
                ...
                bitZeroZOnly: convertToImperial(
                    touchplate.zThickness.bitZeroZOnly ?? 0,
                ),
            };
    ...
    const onStoreChange = useCallback(
        (data: { workspace: any }) => {
            ...
            if (
                ...
                setToolDiameter(
                    calcToolDiameter(newUnits, PROBE_TYPE_DIAMETER),
                );
            } else {
                        ...
                        <button
                            ...
                            onClick={() => {
                                ...
                                actions._setToolDiameter({
                                    value: lastProbeDiameter,
                                });
                                if (lastProbeType)
                                    actions._setProbeType(
                                        lastProbeType as PROBE_TYPES_T,
                                    );
