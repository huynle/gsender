...
export default function ConnectionWidget() {
    ...
    const handleConnect = useCallback(
        (portName: string, type: ConnectionType) => {
            ...
            const defaultFirmware = store.get(
                'workspace.defaultFirmware',
                GRBL,
            );
            const ethernetPort = store.get(
                'widgets.connection.ethernetPort',
                23,
            );
    ...
    useEffect(() => {
        const token = pubsub.subscribe('reconnect', () =>
